      ...
      var reads = 0, writes = 0,
        ...
        process = function process(data, cb) {
          ...
          async.series(loaders, function (err) {
            ...
            var periodics = [];
            for (var target in self.op.interval) for (var interval in self.op.interval[target]) {
              if (writes % interval !== 0) continue; // not currently on this interval

              if (!util.isArray(self.op.interval[target][interval])) {
                self.op.interval[target][interval] = [self.op.interval[target][interval]];
              }

              for (var k in self.op.interval[target][interval]) {
                periodics.push(
                  self.op.interval[target][interval][k].bind(self, self.db[target], data, registry)
                );
              }
            }
