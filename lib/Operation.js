  ...
  for (var name in self.op.extract) for (var i in self.op.extract[name]) {
    ...
    queue.push(function (next) {
      ...
      var process = function process(data, callback) {
        reads++;
        if (self.debug) console.log((self.op.name || 'Operation') + ' : Read ' + reads);
 
        self.op.transform[dbName](self.db[dbName], registry, data, function load(err, data) {
          ...
          async.series(loaders, function (err) {
            ...
            if (self.debug) console.log((self.op.name || 'Operation') +
              ' : write ' + writes + ' ' + ((writes/reads) * 100).toFixed(2) + '% ' + 
              (writes / ((new Date().getTime() - registry.startDate.getTime()) / 1000)).toFixed(1) + ' r/s');
          });
        });
      };
