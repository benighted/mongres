  ...
  for (var name in self.op.extract) for (var i in self.op.extract[name]) {
    ...
    queue.push(function (next) {
        ...
        proc = function proc(data, cb) {
          ...
          async.series(loaders, function (err) {
            ...
            for (var interval in self.op.interval) for (var target in self.op.interval[interval]) {
              if (writes % interval !== 0) continue; // only run if no remainder 

              // wrap function in an array if is not already an array
              if (!util.isArray(self.op.interval[interval][target])) {
                self.op.interval[interval][target] = [self.op.interval[interval][target]];
              }

              // queue intervallic functions
              for (var k in self.op.interval[interval][target]) intervals.push(
                self.op.interval[interval][target][k].bind(self, self.db[target], registry)
              );
            }

            // execute intervallic functions in series
            async.series(intervals, cb);
          });
        },
      ...
      return self.op.extract[source][index](
        self.db[source], registry, proc, done
      );
    });
  }
