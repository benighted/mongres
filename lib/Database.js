...
Database.prototype.connect = function (callback) {
  ...
  if (self.connecting) { // connection in progress, try again
    return setTimeout(self.connect.bind(self, callback), 100);
  } else if (self.client && self.connected) { // is connected
...
Database.prototype.query = function (query, options, callback) {
  return this.queryArray(query, options, callback);
};
...
Database.prototype.queryStream = function (query, options, callback) {
  if (arguments.length === 2) { // options object is optional
    callback = options;
    options = {};
  }
  ...
  switch (this.type) {
    case DB_TYPE_MONGODB:
      for (var collection in query) {
        var cursor = this.client.collection('sync')
          .find(query[collection], options);
    ...
    case DB_TYPE_POSTGRESQL:
      ...
      // stream.on('end', this.client.close);
      process.nextTick(callback.bind(this, null, stream));
