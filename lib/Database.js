...
Database.prototype.connect = function (callback) {
  ...
  if (self.verbose) console.info('Connecting to ' + self.alias +
    ...
    case DB_TYPE_MONGODB:
      ...
      mgClient.open(function(err, client) {
        ...
        if (err) {
          ...
          self.client = db;
          self.connected = true;
          if (self.verbose) console.info('Connected to ' + self.alias +
            ' at ' + self.host + ':' + self.port);
  ...
  switch (this.type) {
    case DB_TYPE_MONGODB:
      for (var collection in query) {
        ...
        else return stream; // only the first query will execute if no callback
      }
